...
/**
 * This is a custom error controller to handle error/exception types which are
 * application specific with error details in response body added.
 * 
 * @author mdodia
 * 
 */
...
public class LocationErrorController implements ErrorController {
	...
	ErrorResponse error(HttpServletRequest request, HttpServletResponse response) {
		...
		return new ErrorResponse(response.getStatus(), getErrorAttributes(request, debug));
	}
